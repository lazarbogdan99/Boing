...
public class BouncingShapesWindow {
    ...
    /**
     * this method decides if the shape has to pulse
     * what shape is it
     * seting the minimum and maximum size of pulsing
     * when the shape reaches the max value, it starts to get smaller
     * when the shape reaches the min value, it starts to get bigger
     */
        ...
        for (ClosedShape s : activeShapes)
        {
            ...
            if(s.isPulsing()){
                if(s.getShape().equals("circle")){
                    ...
                    if(circle.getDiameter() < 2.2*circle.getFirstDiameter() && circle.isMoving()) {
                        circle.setDiameter(circle.getDiameter() + 1);
                    }
                    else if(circle.getDiameter() > circle.getFirstDiameter()) {
                        circle.setDiameter(circle.getDiameter() - 1);
                    ...
                    Rect rect = (Rect)s;
                    if(rect.getHeight()*rect.getWidth() < 2.4*rect.getFirstHeight()*rect.getWidth() && rect.isMoving()){
                        rect.setHeight(rect.getHeight() + 1);
                        rect.setWidth(rect.getWidth()+1);
                    }
                    else if(rect.getHeight()*rect.getWidth()>rect.getFirstHeight()*rect.getWidth()) {
                        rect.setHeight(rect.getHeight() - 1);
                        rect.setWidth(rect.getWidth()-1);
                    ...
                    Oval oval=(Oval) s;
                    if(oval.getHeight()*oval.getWidth()<2.3*oval.getFirstHeight()*oval.getFirstWidth() && oval.isMoving()){
                        oval.setHeight(oval.getHeight()+1);
                        oval.setWidth(oval.getWidth()+1);
                    }
                    else if(oval.getHeight()*oval.getWidth()>oval.getFirstHeight()*oval.getFirstWidth()){
                        oval.setHeight(oval.getHeight()-1);
                        oval.setWidth(oval.getWidth()-1);
                    ...
                    if(triangle.getSide()<2.2*triangle.getFirstSide() && triangle.isMoving()) {
                        triangle.setSide(triangle.getSide() + 1);
                    }
                    else if(triangle.getSide()>triangle.getFirstSide()){
                        triangle.setSide(triangle.getSide()-1);
                    ...
                    if(square.getSide()<2.1*square.getFirstSide() && square.isMoving()) {
                        square.setSide(square.getSide() + 1);
                    }
                    else if(square.getSide()>square.getFirstSide()){
                        square.setSide(square.getSide()-1);
